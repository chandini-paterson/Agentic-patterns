...
def display_message(message: Dict):
    ...
    with st.container():
        st.markdown(f'<div class="query-text">🤔 {message["query"]}</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="response-text">💬 {message["response"].response}</div>', unsafe_allow_html=True)
...
def main():
    ...
    with col1:       
        # Create a container for message with custom height
        ...
        # Process query on submit or Enter key
    ...
    with st.sidebar:
        ...
        if st.session_state.messages:
            ...
            for msg in st.session_state.messages:
                # Category stats
                cat = msg['routing_decision'].category
                category_counts[cat] = category_counts.get(cat, 0) + 1
                
                # Confidence stats
                total_confidence += msg['routing_decision'].confidence
                
                # Handler stats
                handler = msg['response'].handler_name
                handler_counts[handler] = handler_counts.get(handler, 0) + 1
            
            st.metric("Total Queries", len(st.session_state.messages))
            st.metric("Avg Confidence", f"{total_confidence/len(st.session_state.messages):.1%}")
            
            # Category distribution
            st.subheader("Category Distribution")
            for category, count in sorted(category_counts.items(), key=lambda x: x[1], reverse=True):
                percentage = (count / len(st.session_state.messages)) * 100
                st.progress(percentage / 100)
                st.write(f"{category.value}: {count} ({percentage:.1f}%)")
        else:
            st.write("No queries processed yet.")
